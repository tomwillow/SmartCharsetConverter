...
// 双向map，支持双向查表操作
template <typename T1, typename T2> class doublemap {
public:
    ...
    bool has(const T1 &t1) const { return t1ToT2.find(t1) != t1ToT2.end(); }

    bool has(const T2 &t2) const { return t2ToT1.find(t2) != t2ToT1.end(); }
    ...
    const T2 &operator[](const T1 &t1) const { return t1ToT2.at(t1); }

    const T1 &operator[](const T2 &t2) const { return t2ToT1.at(t2); }
